"""Pygame-based renderer for the cart-pole RDDL environments.

Adapted from the pyRDDLGym-project rddlrepository CartPole_Discrete
visualizer so it can be reused and extended independently (e.g. for
recording rollouts or plotting reward curves alongside the render).
"""
import numpy as np
...
class CartPoleVisualizer(BaseViz):
    ...
    def render(self, state):
        ...
        world_width = self._model.non_fluents['POS-LIMIT'] * 2
        ...
        # draw the pole
        polewidth = 10.0
        polelen = scale * (2 * self._model.non_fluents['POLE-LEN'])
